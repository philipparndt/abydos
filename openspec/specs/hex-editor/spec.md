...
A file looked at as bytes: a caret and a selection over them, a find bar that searches bytes, edits in overwrite by default and saved through a temporary, an inspector that says what the bytes at the caret are, and a minimap, an entropy curve, a structure tree for known formats and an answer from Claude beside them — all at the cost the memory-mapped viewer already paid.
## Requirements

...
A tool Xcode owns SHALL come from Xcode, not from the PATH.
...
One search SHALL find every tool this program runs.
...
A language server SHALL be kept until the app goes, and no longer.
...
There SHALL be one server per project per server, not one per language.
...
What is running SHALL be visible, and SHALL be stoppable.
...
A project SHALL choose which server answers for a language.
...
A chosen server that cannot be started SHALL say so.
...
Choosing where a server comes from SHALL take effect now.
...
The footer SHALL say which server is answering, and from where.
...
A server that started and is not answering SHALL say so above the file.
...
A server SHALL be able to change the code, and rename SHALL be what it is asked for.
...
A rename that cannot be offered SHALL NOT be offered.
...
A rename SHALL say which kind of rename it is.
...
A workspace edit SHALL be worked out in full before anything is written.
...
A workspace edit SHALL reach open documents through the rope, and SHALL reach files that move.
...
A whole rename SHALL be one undo.
...
A server that declines after offering SHALL be named.
...
A project or a person SHALL be able to name the executable for a server.
...
A project SHALL be able to say what to tell a server when it starts.
...
### Requirement: A completion is inserted as text, and its stops are stepped through

A completion SHALL be inserted as text, and its stops SHALL be stepped through.
...
The Swift indexer SHALL build outside the project, and SHALL stand outside it too.
...
### Requirement: Keeping a server costs no other tool its turn

Keeping a server SHALL cost no other tool its turn.

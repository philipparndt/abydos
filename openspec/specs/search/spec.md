...
A mark SHALL survive the search being run again.
...
Marks SHALL be hidden on request, so that the list can also empty.
...
⌘Z in the results list SHALL take back the last marking.
...
The results list SHALL be able to be worked from the keyboard.
...
Search results SHALL go wherever a usages list goes.
...
The window SHALL go on answering while a broad search runs.
...
A list that is not the whole answer SHALL say so.

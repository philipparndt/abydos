...
Values in dotenv-shaped and decrypted secret files are drawn under an opaque cover from the moment the file opens, revealed only by an explicit act, so a shared screen never shows a key that was open before the call was.
## Requirements

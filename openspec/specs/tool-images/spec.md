...
Language servers and other tools that come from a container image rather than from the PATH: how an image is chosen, fetched, built here from a recipe Abydos ships, watched while it is slow, and cleaned up after a run that died.

## Requirements

### Requirement: A tool can come from an image, and the project's choice wins

A tool SHALL be able to come from an image, and the project's choice SHALL win.
...
An image that is not here SHALL be fetched before the tool is first used.
...
A server in a container SHALL talk about files by their names on this machine.
...
A published image SHALL only be offered once somebody has run it.
...
Every language server this editor offers SHALL have an image this repository builds.
...
A tool SHALL be able to be built here from a recipe Abydos ships.
...
An edited recipe SHALL rebuild, and an unedited one SHALL never rebuild.
...
Nothing built here SHALL ever be fetched from a registry.
...
A build that fails SHALL say which kind of failure it was.
...
A server SHALL be allowed to say it reads directories the project does not contain, and that SHALL NOT be treated as a failure.
...
An image that takes minutes SHALL be watched rather than waited for.
...
A project pinning a toolchain the image has not got SHALL be told before anything starts.
...
A recipe that is not the tool's own SHALL be able to be asked for.
...
A container an earlier run left behind SHALL be removed by the next one, in whichever runtime holds it.

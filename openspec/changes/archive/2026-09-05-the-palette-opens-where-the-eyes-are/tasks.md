...
- [x] 1.1 `ProjectSwitcherPopover.show` takes where it is to appear — anchored
...
- [x] 1.2 The centred path uses `PalettePanel` with the placement
...
- [x] 3.1 A `placementForTesting` for the switcher's panel in the shape
  ...
  screenshot.
- [x] 3.2 A driven run: ⇧⌘P opening centred over a wide window and over a
...
- [x] 4.1 `docs/release-notes-0.14.0.md` given the paragraph, and
...
- [x] 4.2 Green by their exit codes: `make test` 4090 tests in 520 suites,
  exit 0 with the suite's two standing known issues, load 60.4 over 10 cores;

...
- [x] 1.1 `MainWindowController.discard(paths:subject:)`: the insurance, the
...
- [x] 1.2 The estate from the window, not the pane, so the tree can ask with the
      pane closed.

## 2. The menu

- [x] 2.1 *Discard Changes* in `ProjectNavigator+Menu.swift` after *Add to
      .gitignore…*, its title from `GitDiscard.menuTitle` for the selected rows.
- [x] 2.2 `menuNeedsUpdate`: hidden unless a selected row's status — a folder's
      aggregate — is neither unmodified nor ignored, and hidden over a conflict.
- [x] 2.3 The action: `discardable` for the tree's rows — paths, the changes
      under them, the subject — the `GitDiscard` alert, and the shared verb on
      the destructive button.

## 3. Proving it

- [x] 3.1 Tree steps `discard` (prints title and question, discards nothing)
      and `discard-confirm` (discards on the scratch copy, prints the file's
      porcelain status).
- [x] 3.2 Driven on a scratch checkout: a modified file, a folder with two

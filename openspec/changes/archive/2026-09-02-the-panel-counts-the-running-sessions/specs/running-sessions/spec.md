...
What the app knows about the Claude Code sessions running anywhere on this
machine, for this user, and how the terminal panel summarises and lists them.
The knowledge comes from the hook, which announces every event of every session
as a distributed notification; nothing here reads a disk or asks a process.

The vocabulary is the tmux tabs': a session is **working**, **needs input** or
**done**, and a working session that has said nothing for thirty seconds is
shown as **unknown**. This capability adds no fourth state and no second
staleness bound.

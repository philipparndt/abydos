...
- [x] 1.1 Add `Sources/AbydosKit/Debug/BreakpointRows.swift`: a row value —
...
- [x] 1.2 Add `Tests/AbydosKitTests/BreakpointRowTests.swift`: three files come
...
- [x] 2.1 `DebugPane.session` becomes `DebugSession?`. Every one of the
...
- [x] 2.2 `DebugPane.debugSession` becomes optional and `isSessionActive` false
...
- [x] 2.3 `BottomPanel.showDebug()` makes the pane when there is none, rooted at
      ...
      panel and it.
- [x] 2.4 Check that starting a session while the empty pane is open leaves one
      debug pane, not two: `makeDebugSession` already closes an existing one.

## 3. The Breakpoints tab

- [x] 3.1 A segmented control for the left-hand side — Stack | Breakpoints —
...
- [x] 3.2 `BreakpointList`, an `NSView` over an `NSTableView`, drawing the rows
...
- [x] 3.3 The code for a row: the editor's document when the file is open, the
...
- [x] 3.4 Row verbs, each through `DebugCoordinator` so a running session is told
...
- [x] 3.5 `DebugCoordinator` gains a hook that says the set changed, fired from
...
- [x] 4.1 `RunControl.showStartMenu()` gains Debug Executable… and Attach to
...
- [x] 4.2 Wire the three in `MainWindowController` to `debugExecutable`,
      `attachToProcess` and `goDebug`, which do not move.
- [x] 4.3 `ToolWindowBar.debugButtonPressed` becomes `onToggleDebug?()` and
...
- [x] 4.4 Check the Run menu's own Attach to Process… still validates and still
      works: it is not the rail's and does not move.

## 5. Driving it, and finishing

- [x] 5.1 A driven-run verb that opens the debug pane with nothing running and
...
- [x] 5.2 Drive it against a copy under the scratchpad, with a throwaway bundle
      ...
      Never `make install`.
- [x] 5.3 `make test` and `make warnings`, both clean, and their exit codes read

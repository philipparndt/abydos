...
- **The ceiling is 14 lines, counted rather than guessed.** Against the macOS
  27.0 SDK, `NSResponder.h` declares 43 methods beginning `move` or `select` and
  `doCommand` handles 29:
  ...
  The count reproduces with a `grep` over that header, which the design carries.
  It is the size of a family AppKit fixes at compile time, not a function of how
  long the app runs.

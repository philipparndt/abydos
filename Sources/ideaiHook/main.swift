...
default:
	...
	ideai-hook — tells ideai what Claude Code sessions are doing

	  ideai-hook            run as a hook (reads the event JSON on stdin)
	  ideai-hook install    wire it into ~/.claude/settings.json
	  ideai-hook remove     take it back out
	  ideai-hook status     say whether it is wired up

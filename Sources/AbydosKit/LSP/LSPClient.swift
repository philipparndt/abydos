...
public final class LSPClient: @unchecked Sendable {
	...
	/// One entry of what the server said it can do, read under the lock.
	///
	/// The questions in `LSPClient+Questions.swift` all begin by asking whether
	/// the server offers the thing at all, and this is what they ask with. One
	/// entry rather than the dictionary, because a caller holding the whole of
	/// `capabilities` has taken a copy that the next `initialize` will not
	/// update.
	func capability(_ name: String) -> Any? {
		locked { capabilities[name] }
	}

	/// The whole of what the server said it can do, for the two readings that
	/// are about a provider's options rather than its presence.
	var allCapabilities: [String: Any] {
		locked { capabilities }
	}

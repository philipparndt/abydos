...
final class TerminalMetalRenderer {
	...
	///
	/// Kept in the body because a stored property cannot live in an extension.
	...
	var imageTextures: [UInt32: MTLTexture] = [:]
	/// What the store's generation was when the textures were last checked.
	var lastGraphicsGeneration = -1
	/// This frame's pictures, split by whether they go under the text or over it.
	var imagesBelow: [(payload: ImageInstance, texture: MTLTexture)] = []
	var imagesAbove: [(payload: ImageInstance, texture: MTLTexture)] = []

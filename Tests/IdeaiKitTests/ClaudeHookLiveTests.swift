...
struct ClaudeHookLiveTests {
	...
	private var hook: URL? {
		let root = URL(fileURLWithPath: #filePath)
			.deletingLastPathComponent()   // IdeaiKitTests
			.deletingLastPathComponent()   // Tests
			.deletingLastPathComponent()   // the package
		let build = root.appendingPathComponent(".build")
		let candidates = [
			build.appendingPathComponent("debug/ideai-hook"),
			build.appendingPathComponent("release/ideai-hook"),
		] + ((try? FileManager.default.contentsOfDirectory(atPath: build.path)) ?? [])
			.filter { $0.contains("apple-macosx") }
			.flatMap { architecture in
				["debug", "release"].map {
					build.appendingPathComponent("\(architecture)/\($0)/ideai-hook")
				}
			}
	...
	@Test mutating func aTurnThatEndsStaysEnded() throws {
		...
		let hook = try #require(self.hook, "run `swift build` so ideai-hook is beside the tests")
		let session = "ideai-hook-live-\(UUID().uuidString.prefix(8))"

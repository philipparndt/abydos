...
struct LaunchStoreTests {
	...
	@Test func writesOneFilePerConfiguration() throws {
		...
		let files = try FileManager.default.contentsOfDirectory(
			atPath: IdeaiFolder.runDirectory(in: root).path
		).sorted()
	...
	@Test func keepsOneFilePerName() throws {
		...
		let copy = IdeaiFolder.runDirectory(in: root).appendingPathComponent("copy.json")
...
struct IdeaiFolderTests {
	@Test func commitsTheRunConfigurationsAndNothingElse() throws {
		...
		let ignore = try String(
			contentsOf: IdeaiFolder.url(in: root).appendingPathComponent(".gitignore"),
			encoding: .utf8
		)
		...
		#expect(listed.contains(".ideai/run/keep-me.json"))
	...
	@Test func leavesAnEditedGitignoreAlone() throws {
		...
		_ = try IdeaiFolder.create(in: root)
		let ignore = IdeaiFolder.url(in: root).appendingPathComponent(".gitignore")

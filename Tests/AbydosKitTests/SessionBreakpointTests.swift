...
struct SessionBreakpointTests {
	...
	@Test func writesThemInAStableOrder() throws {
		...
		try SessionStore.write(session, in: root)
		let first = try String(contentsOf: AbydosFolder.sessionFile(in: root), encoding: .utf8)

		try SessionStore.write(session, in: root)
		let second = try String(contentsOf: AbydosFolder.sessionFile(in: root), encoding: .utf8)
		#expect(first == second)
	...
	@Test func aFileWithoutBreakpointsLeavesNothingBehind() throws {
		...
		let text = try String(contentsOf: AbydosFolder.sessionFile(in: root), encoding: .utf8)

...
struct PortForwardTests {
	...
	@Test func forwardsToARealPod() async throws {
		...
		guard let namespace = environment["ABYDOS_K8S_NAMESPACE"],
		      let name = environment["ABYDOS_K8S_POD"]
		else { return }
		...
		let target = PodTarget(
			...
			phase: "Running",
			containers: [],
			port: Int(environment["ABYDOS_K8S_PORT"] ?? "6060") ?? 6060,
			portSource: .convention
		)
		let forward = try await PortForward.start(to: target, context: environment["ABYDOS_K8S_CONTEXT"])
		defer { forward.stop() }

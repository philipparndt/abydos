...
## Re-landed under 0491, and what happened to the symptom it went out on

Reverted on 2026-08-14 for two reasons, and neither survived being measured again.

**The frozen screen was not this.** The `renders=1 parse=690ms build=2ms` that the
revert quoted was measured on libghostty-vt, against a `renders=43 parse=505ms
build=250ms` measured on our engine — the throwaway defaults domain had inherited
`terminalGhosttyEngine` from the machine's own settings, which is a trap 0491
records. On one binary with this merge *absent*, our engine gives 38–45 renders and
libghostty-vt gives 1, and the second is the revert's own number to the millisecond.
`build 250 → 2ms` was likewise not the row cache: 250 ms over 43 renders and 5 ms
over 1 are both 5.8 ms a frame.

**And the stale text under cursor-up did not reproduce.** Three attempts, all on this
merge with 0491's policy in, our engine, GPU path, screen captured and read:

- ten rewrites of a row under `ESC[1A ESC[2K`, printing a different marker each time
  — only the last marker on screen, which is correct;
- a prompt line rewritten in place the way `zle` does it — `\r`, the prompt, the
  recalled command, `ESC[K`, cursor left on the row — eight times, with the commands
  getting shorter and longer in turn so that a kept row would leave a tail. No tail;
- **a real login shell and a real up arrow**, recalling a command long enough to wrap
  the prompt line. Drawn correctly, cursor at the end of it.

So it is unreproduced rather than fixed, and that is written here rather than ticked.
What is fixed is the thing that would look identical to somebody using the app: a
terminal drawn once a second shows the previous picture for up to a second after
every keystroke, and that is what the pane was doing when the report was made.

**`abydos-bench --mode prompt` is the pattern that was missing**, and it is the one
this item's own revert asked for. It rewrites the prompt line under cursor-up at ten
frames a second, with the recalled command changing length every frame, so a kept row
that missed an invalidation is visible as text the emulator does not have. One binary,
`ABYDOS_METAL_ROW_CACHE=0` against unset, 1920×1050, 235×47, load 7.2–9.8 over ten
cores:

| `prompt` | renders | cells/render | rows/render | instances/render | build |
|---|---|---|---|---|---|
| every row, as before | 10 | 11,045 | 47 | 11,045 | 53–56 ms (5.5 ms/frame) |
| only what changed | 10 | **235** | **1** | 11,045 | **3 ms (0.3 ms/frame)** |

**47× fewer cells and 18× less build**, on a shell rewriting its prompt line — which
is the same ratio this item measured on `status` and is now measured on the pattern
it was reverted over. `instances/render` stays at the whole screen, which is the proof
the rows nobody rebuilt are still there. And the picture is right: photographed
mid-run, the prompt line reads `prompt$ echo xxx…-104` with no tail from the longer
command before it.

`plain` and `fire` are unchanged by the cache and now both render at 60: 46.1 against
46.4 MB/s and build 116 against 123 ms with the cache off and on, because a screenful
of log and a screenful of fire change every row of every frame. That was this item's
finding and it still is.

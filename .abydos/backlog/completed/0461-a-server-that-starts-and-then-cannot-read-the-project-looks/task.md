...
## Settled at the merge: 0462 speaks first

0461 and 0462 were worked in parallel against the same broken project, and both
had something to say about it. 0462 wins there, and the order in `notice` says
so: a pin is read *before* anything is started, so it can name the toolchain, the
file that named it and what would read the project, while this item only knows
that the server complained and can quote it. Where both would fire the one that
names the reason is the better sentence.

Which leaves this state doing what it is actually for — everything that cannot be
known ahead of the server trying — and it means the toolchain scenario written
here belongs to 0462 now. `spec/language-servers.md` carries the exchange: a
server that exited on the way up, and a pin, which is answered earlier.

...
`spec/` had no `devcontainers.md` when this was written, so everything below is
`ADDED` — including the parts 0433 and 0438 built and this item only changed.
Writing a `MODIFIED` against a requirement nobody can find would be a delta that
does not fold, and half a capability is worse than none: a reader who found only
"a project with several containers can be switched between them" would have no
way to know what a project with one does. What is here is what the program does
today, with the four things 0444 changed among them.

## ADDED Requirement: A devcontainer is asked for before it is started
...
## ADDED Requirement: The answer says which container, and is kept per project
...
## ADDED Requirement: An answer naming a container the project no longer offers is a question again
...
## ADDED Requirement: The titlebar says a container is in use, not which one
...
## ADDED Requirement: The pill's menu lists the containers and switches between them
...
## ADDED Requirement: Bringing a container up is visible where it is slow

<!-- What this item changes about `language-servers`. Folded into
     ...
     Nothing has been said about language-servers yet, so this is all ADDED.
-->

## ADDED Requirement: A tool Xcode owns comes from Xcode, not from the PATH
...
## ADDED Requirement: One search finds every tool this program runs
...
## ADDED Requirement: A language server is kept until the app goes, and no longer
...
## ADDED Requirement: One server per project per server, not per language
...
## ADDED Requirement: What is running can be seen and stopped

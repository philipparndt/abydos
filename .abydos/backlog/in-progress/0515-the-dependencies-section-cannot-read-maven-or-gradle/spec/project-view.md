<!-- What this item changes about `project-view`. Folded into
     ...
     Maven and Gradle are now read, so three requirements gain the cases
     that answer for them — and one gains a state the section did not
     have before: a list that is real and also incomplete, which is what
     a `pom.xml` and a Gradle build with no lock file can honestly give.
     The Maven row in "a kind this cannot read" becomes a Bazel one,
     since Maven is no longer an example of one.
-->
...
## ADDED Requirement: A list that is read and incomplete says what is missing
